...
const PortfolioApp = {
        ...
        try {
            ...
            projects.forEach((project, index) => {
                ...
                if (videoId) {
                    ...
                    this.refineOrientationFromCover(videoBox, project, null, grid);
                } else {
                    videoBox.innerHTML = `
            ...
            this.pairPortraitCards(grid);
        } catch (err) {
            console.error('Error rendering projects:', err);
        }
    },

    pairPortraitCards(grid) {
        if (!grid) return;
        const cards = Array.from(grid.querySelectorAll('.project-card'));
        cards.forEach((card) => card.classList.remove('is-portrait-solo'));

        let pending = null;
        cards.forEach((card) => {
            if (card.classList.contains('is-portrait')) {
                pending = pending ? null : card;
            } else if (pending) {
                pending.classList.add('is-portrait-solo');
                pending = null;
            }
        });
        if (pending) pending.classList.add('is-portrait-solo');
    },
    ...
    refineOrientationFromCover(element, item, extraEl, grid) {
        ...
        img.onload = () => {
            ...
            if (grid) this.pairPortraitCards(grid);
        };
